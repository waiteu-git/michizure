 ...
 *
 * ⚠ 1 を残してあるのは、既に 1 で作られた部屋があれば消すと二度と開かなくなるため。
 *   **2026-09-18 に本番デプロイ済み**なので「一度もデプロイしていない」を理由にはもう削除できない。
 *   削除してよい条件は**本番に kdfVersion 1 の部屋が一つも存在しないことを人間が確認できたら**
 *   （既定値は2なので新規の部屋が1になることは無いが、確認自体は人間の手＝Cloudflare の
 *   Durable Objects ダッシュボードが要る）。
 */
